...
class Trainer:
    ...
    def _train_step(bar: tqdm, model: Model, device: torch.device, train_dl: data.DataLoader,
                    loss_fn: Loss, optimizer: Optimizer, history: History, step_scheduler):
        model.train()
    ...
    def _train(self, epochs: int, loss_fn: Loss, o: Optimizer, step_scheduler) -> Tuple[History, History]:
    ...
    def find_lr(self, start_lr: float, end_lr: float, max_steps: int = 100, wd: float = 0.0) -> LRHistory:
        ...
        with tqdm(total=min(max_steps, history.steps), desc=f'Finding LR in {max_steps} steps') as bar:
            self._train_step(bar, self.model, self.device, self._train_dl, self.model.loss_fn, o,
                             history, lr_schedule)
        # Restore model and optimizer
        logger.info(f'Restoring model from {save_file}')
        self.model.load(save_file)
        os.remove(save_file)
        return history
    ...
    def train_one_cycle(self, epochs: int, max_lr: float, wd: float = None, pct_start: float = 0.3,
        ...
        Args:
            ...
            max_lr: Maximum learning rate to use.
            wd: The weight decay. Default depends on the model.
